<template>
    <div class="container my-5" data-aos="fade-up" data-aos-duration="1500">
      <h1 class="text-center titleColor mb-5">Encuentra el Au Pair ideal</h1>
      <b-card class="p-2 mb-5 shadow custom-card">
        <b-form @submit.prevent="onSubmit">
          <b-row>
            <b-col cols="12" md="6">
              <b-form-group label="Vivimos en *" label-for="country">
                <b-form-select v-model="form.country" :options="countries" required></b-form-select>
              </b-form-group>
            </b-col>
            <b-col cols="12" md="6">
              <b-form-group label="Buscamos *" label-for="gender">
                <b-form-radio-group v-model="form.gender" :options="genders" required></b-form-radio-group>
              </b-form-group>
            </b-col>
          </b-row>
          <b-row class="justify-content-center">
            <b-form-group label="Me gustaría encontrar un au pair en *" label-for="destination">
          ...
          <b-button type="submit" class="global-button  mt-2 px-5">Buscar</b-button>
        </b-form>
      </b-card>
  
      <div v-if="auPairs.length" class="mt-4">
        <h4 class="text-center mb-4">{{ auPairs.length }} au pairs de {{ totalAuPairs }} encajan con lo que buscas</h4>
        <b-row>
          <b-col v-for="aupair in auPairs" :key="aupair.id" cols="12" md="6" lg="4">
            <b-card class="mb-4 shadow-sm rounded-lg" :title="aupair.nameHost" :img-src="aupair.image" img-alt="Family Image" img-top>
              <b-card-text>
                <p><strong>{{aupair.nameAuPair }}</strong></p>
                <p><strong>Ubicación:</strong> {{ aupair.location }}</p>
                <p><strong>Rango de edad de niños a cuidar:</strong> {{ aupair.childrenAgeMin }}-{{ aupair.childrenAgeMax }}</p>
                <p><strong>Comienzo:</strong> {{ aupair.availableFrom }} - {{ aupair.availableTo }}</p>
                <p><strong>Duración:</strong> {{ aupair.minStayMonths }} - {{ aupair.maxStayMonths }} meses</p>
                <p>{{ aupair.description }}</p>
              </b-card-text>
            </b-card>
          </b-col>
        </b-row>
  ...
  <script>
  import auPairService from '../../services/AuPair/aupair-services';
    ...
    data() {
      return {
        ...
        genders: [
          ...
          { text: 'un chico y una chica au pair', value: 'Ambos' },
        ],
        destinations: [
    ...
    methods: {
      ...
      async onSubmit() {
        ...
        const endDate = new Date(endYear, endMonth - 1);
          ...
          const formData = {
            familyCountry: this.form.country,
            genderSearch: this.form.gender,
            preferredCountryNames: this.form.destination,
            startDate: startDate,
            endDate: endDate,
            minDuration: this.form.minDuration,
            maxDuration: this.form.maxDuration
          };
          const response = await auPairService.findAuPair(formData);
          console.log(response);
          this.auPairs = response.data;
        } catch (error) {
